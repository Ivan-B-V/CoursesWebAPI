...
public sealed class ActivityRepository : RepositoryBase<Activity>, IActivityRepository
{
    ...
    public async Task<PageList<Activity>> GetByParametersAsync(ActivityQueryParameters queryParameters, CancellationToken cancellationToken = default)
    {
        ...
        if (!string.IsNullOrEmpty(queryParameters.ActivityType))
        {
            activitiesQuery = activitiesQuery.Where(a => a.ActivityType.Name.Contains(queryParameters.ActivityType));
            //EF.Functions.ILike(c.ActivityType.Name, $"%{searchTerm}%") ||
        }
        ...
        activitiesQuery = activitiesQuery.Search(queryParameters.SearchTerm)
    ...
    public async Task<IEnumerable<Activity>> GetByExpressionAsync(Expression<Func<Activity, bool>> expression, bool trackChanges, CancellationToken cancellationToken = default)
    {
